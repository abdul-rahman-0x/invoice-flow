...
// Define a safe type extension for jsPDF to prevent ESLint 'no-explicit-any' violations
...
export const calculateTotals = (
    ...
    // Added safe numerical fallbacks to prevent NaN errors during live typing
...
export const generateInvoicePDF = (
    ...
    // Safe cast without using 'any' to preserve TypeScript rules
    ...
    doc.text(`$${totals.subtotal.toFixed(2)}`, pageWidth - 15, totalsY, {
        align: "right",
    });

    totalsY += 6;
    doc.text(`Tax (${data.taxRate || 0}%):`, totalsX, totalsY);
    doc.text(`$${totals.taxAmount.toFixed(2)}`, pageWidth - 15, totalsY, {
        align: "right",
    });
    ...
    doc.text(`$${totals.total.toFixed(2)}`, pageWidth - 15, totalsY, {
        align: "right",
    });
